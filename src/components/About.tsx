        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="relative">
              {/* Profile picture holder with consistent dark grey theme */}
              <div className="w-80 h-80 mx-auto bg-gray-900 border-2 border-gray-800 rounded-full flex items-center justify-center shadow-2xl shadow-gray-900/50">
                <div className="w-72 h-72 bg-gray-800/50 border border-gray-700 rounded-full flex items-center justify-center">
                  <div className="text-6xl font-bold text-gray-400">AT</div>
                </div>
              </div>
              
              {/* Floating elements with consistent palette */}
              <motion.div
                animate={{ rotate: 360 }}
                transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                className="absolute top-4 right-4 w-16 h-16 border-2 border-gray-700/50 rounded-full"
              />
              <motion.div
                animate={{ rotate: -360 }}
                transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                className="absolute bottom-4 left-4 w-12 h-12 border-2 border-gray-800/80 rounded-full"
              />
            </div>
          </motion.div>
          ...
          >
            ...
            <p className="text-gray-400 text-lg leading-relaxed">
              I'm a Software Developer with a deep passion for backend development 
            ...
            <p className="text-gray-400 text-lg leading-relaxed">
  When I’m not deep in code, you might catch me geeking out over the latest AI/ML breakthroughs, sneaking commits into open-source projects at 2 AM. And when my brain needs a reboot, you’ll probably find me at the gym pretending I know what I’m doing, hunting for the city’s best coffee, or plotting my next unnecessarily over-engineered side project.
            </p>

            {/* Skill tags with consistent dark theme */}
            <div className="flex flex-wrap gap-4 mt-8">
              {['Python', 'FastAPI', 'AI/ML', 'Backend Architecture', 'Cloud Computing'].map((skill) => (
                <span
                  key={skill}
                  className="px-4 py-2 bg-gray-800 border border-gray-700 rounded-full text-gray-300 text-sm font-medium"
                >
                  {skill}
                </span>
              ))}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {traits.map((trait, index) => (
              <motion.div
                ...
                whileHover={{ scale: 1.05, borderColor: '#6B7280' }} // gray-500 on hover
                className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-6 transition-colors duration-300"
              >
                <trait.icon className="text-gray-400 mb-4" size={32} />
                <h4 className="text-xl font-semibold text-white mb-3">{trait.title}</h4>
                <p className="text-gray-500">{trait.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
